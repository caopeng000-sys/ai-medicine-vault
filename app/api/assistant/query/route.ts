...
export function createAssistantQueryHandler(resolveQuery: ResolveAssistantQuery = (ctx, q, o) => resolveAssistantQuery(ctx, q, {}, o), getContext = requireCurrentUser, saveConversation = saveAssistantConversation) {
  return async (request: Request) => {
    try {
      const ctx = await getContext(); const body = (await request.json()) as AssistantQueryRequestBody; const question = body.question?.trim()
