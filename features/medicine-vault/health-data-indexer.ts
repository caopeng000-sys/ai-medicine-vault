...
export function indexHealthData(input: {
  ...
  for (const member of input.members) {
    chunks.push({
      ...
      content: [member.name, member.relationship, `${member.birthYear} 年出生`, member.gender, member.note, member.allergySummary]
        .filter(Boolean)
        .join(" · "),
  ...
  for (const record of input.records) {
    ...
    chunks.push({
      ...
      content: [memberName, record.visitedAt, record.hospitalName, record.department, record.diagnosis, record.symptoms, record.doctorAdvice, record.prescriptionNote, record.note]
        .filter(Boolean)
        .join(" · "),
  ...
  for (const medicine of input.medicines) {
    ...
    chunks.push({
      ...
      content: [memberName, medicine.name, medicine.category, medicine.purpose, medicine.instructions, medicine.specification, medicine.usageNote, medicine.safetyNote, `有效期 ${medicine.expiresAt}`]
        .filter(Boolean)
        .join(" · "),
  ...
  for (const allergy of input.allergies) {
    ...
    chunks.push({
      ...
      content: [memberName, allergy.allergen, allergy.severity, allergy.reaction, allergy.discoveredAt, allergy.note]
        .filter(Boolean)
        .join(" · "),
