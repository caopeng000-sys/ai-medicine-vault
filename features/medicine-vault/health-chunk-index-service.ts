...
export async function rebuildIndex(ctx: RepositoryContext, memberId?: string): Promise<HealthChunkIndexResult> {
  const [members, records, medicines, allergies, attachments] = await Promise.all([
    ...
    listMedicalRecordAttachments(ctx, memberId),
  ])
  const chunks = indexHealthData({
    members: memberId ? members.filter((member) => member.id === memberId) : members,
    records,
    medicines,
    allergies,
    attachments,
...
export async function searchChunks(
  ...
  const storedChunks = await listHealthDocumentChunks(ctx, memberId)
  ...
  const indexedChunks = indexHealthData({
    members: memberId ? members.filter((member) => member.id === memberId) : members,
    records,
    medicines,
    allergies,
